...
function InterviewHome({ params, searchParams }: Props) {
  ...
  const organization = { id: "org_tinaai_default" }; // Dummy organization
              ...
              <ScrollArea className="h-full p-1 rounded-md border-none">
                {filterResponses().length > 0 ? (
                  filterResponses().map((response) => (
                    <div
                      className={`p-2 rounded-md hover:bg-indigo-100 border-2 my-1 text-left text-xs ${unwrappedSearchParams.call == response.call_id
                        ? "bg-indigo-200"
                        : "border-indigo-100"
